 ...
 * @return {Promise<string>}
 */
const selectService = (context, namespace) => {
    const podsRaw = cp.execSync(`kubectl get services --namespace ${namespace} --context ${context}`, {encoding: 'utf-8'});
    const serviceSplit = podsRaw.trim().split('\n');
    serviceSplit.shift();
    const services = serviceSplit.map(s => s.trim().split(/\s/).shift());
    return select({question: 'select service', options: services, autocomplete: true});
};
...
module.exports = {getContexts, selectContext, selectNamespace, selectPod, selectService, selectPort};