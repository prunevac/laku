...
const portForward = async () => {
    console.log('lazky kubectl intractive port forward');

    // // context
    const context = await selectContext('context:');
    ...
    const namespace = await select({question:'namespace:', options:namespaces, autocomplete:true});
    ...
    const pod = await select({question:'pod:', options:pods, autocomplete: true});
